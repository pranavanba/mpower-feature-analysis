# Feature QC Diagnostics - Walk 30 Secs
...
get_feature_diagnostics <- function(data, attribute, feature_str){
    tap.2 <- data %>% 
    ...
    feature_set <- tap.2 %>%
        dplyr::select(matches(feature_str)) %>%
        names()
...
get_plots <- function(output_list){
    ...
    ks_test <- output_list$ks_test %>% 
        ...
        dplyr::ungroup() %>%
        ggplot(aes(y = reorder(name, window_median), x = value)) +
...
get_simulation_data <- function(features){
    ...
    # Simulation 0:
    feature_list$no_filter <- features
    
    # Simulation 1 (filter all):
    feature_list$filter_all <- features %>%
...
```{r, include=FALSE}
walk_features_v1 <- synGet(FEATURES$v1)$path %>% 
    fread() %>%
    dplyr::mutate(version_abbv = "V1")
walk_features_v2 <- synGet(FEATURES$v2)$path %>% 
    fread() %>%
    dplyr::mutate(version_abbv = "V2")
walk_features <- dplyr::bind_rows(
    walk_features_v1,
    walk_features_v2) %>%
...
```{r, include=FALSE}
walk_features_sim <-  walk_features %>%
    get_simulation_data()
```

```{r, echo = FALSE, eval = TRUE, include=FALSE}
version_diagnostics <- walk_features_sim %>%
    purrr::map(function(data){
        data %>%
            get_feature_diagnostics(
                attribute = "version_group",
                feature_str = "^x|^y|^z|^AA|^rotation")
    })
```
...
### 2. Filter Early Version (version<1.3)
