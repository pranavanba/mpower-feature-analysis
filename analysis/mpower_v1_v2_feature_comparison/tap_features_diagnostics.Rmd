# Feature QC Diagnostics - Tapping

This wiki page is used to do diagnostic on the tapping features. We are trying to check whether our QC processes are able to clear the version/phoneInfo confounders accross mPower1 and mPower2
 
## QC Steps:
* No Filters
* Filter Timestamps (>20 seconds)
* Filter Tap Interval (<2 maxTapInter)
* Filter Early Version (> Version 1.3)
* Filter NTaps (nTaps < 400 and nTaps > 50)
* Filter All
...
ImportancePlot2 <- function(x) {
    ...
    aux %>% 
        ...
        labs(y = "", 
             ...
             subtitle = "Top 10 Median Impurity")
}
...
get_plots <- function(output_list){
    ...
    ks_test <- output_list$ks_test %>% 
        ...
        labs(x = "-log10(p)", 
             y = "",
             title = "Significant Group Comparisons",
             subtitle = "Ranked by Median P-Value KS-test") +
        theme_minimal()
...
```{r, include=FALSE}
design_layout <- function(plots){
    upper <- (plots$box+plots$conf_mat) + 
        plot_layout(widths = c(2,5))
    bottom <- (plots$importance+plots$ks_test) + 
        plot_layout(widths = c(2,2))
    
    patchwork::wrap_plots(upper, bottom, nrow = 2)
}
```
```{r, include=FALSE}
load(file.path(here::here(),"version_diagnostics.RData"))
load(file.path(here::here(),"phoneInfo_diagnostics.RData"))
```
...
#### b. Phone Info
```{r, fig.width = 15, fig.height = 12, echo=FALSE}
phoneInfo_diagnostics$no_filter %>% 
    get_plots() %>%
    design_layout()
```
...
#### a. Version Groups
```{r, fig.width = 15, fig.height = 12, echo=FALSE}
version_diagnostics$filter_timestamps %>% 
    get_plots() %>%
    design_layout()
```
...
#### b. Phone Info
```{r, fig.width = 15, fig.height = 12, echo=FALSE}
phoneInfo_diagnostics$filter_timestamps %>% 
    get_plots() %>%
    design_layout()
```
...
#### a. Version Groups
```{r, fig.width = 15, fig.height = 12, echo=FALSE}
version_diagnostics$filter_tap_ntaps %>% 
    get_plots() %>%
    design_layout()
```
...
#### b. Phone Info
```{r, fig.width = 15, fig.height = 12, echo=FALSE}
phoneInfo_diagnostics$filter_tap_ntaps %>% 
    get_plots() %>%
    design_layout()
```
...
#### a. Version Groups
```{r, fig.width = 15, fig.height = 12, echo=FALSE}
version_diagnostics$filter_tap_inter %>% 
    get_plots() %>%
    design_layout()
```
...
#### b. Phone Info
```{r, fig.width = 15, fig.height = 12, echo=FALSE}
phoneInfo_diagnostics$filter_tap_inter %>% 
    get_plots() %>%
    design_layout()
```

### 5. Filter Early Version (version<1.3)

#### a. Version Groups
```{r, fig.width = 15, fig.height = 12, echo=FALSE}
version_diagnostics$filter_early_versions %>% 
    get_plots() %>%
    design_layout()
```
...
#### b. Phone Info
```{r, fig.width = 15, fig.height = 12, echo=FALSE}
phoneInfo_diagnostics$filter_early_versions %>% 
    get_plots() %>%
    design_layout()
```
...
#### a. Version Groups
```{r, fig.width = 15, fig.height = 12, echo=FALSE}
version_diagnostics$filter_all %>% 
    get_plots() %>%
    design_layout()
```
...
#### b. Phone Info
```{r, fig.width = 15, fig.height = 12, echo=FALSE}
phoneInfo_diagnostics$filter_all %>% 
    get_plots() %>%
    design_layout()
```